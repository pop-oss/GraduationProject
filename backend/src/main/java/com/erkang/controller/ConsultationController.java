...
public class ConsultationController {
    ...
    public Result<Map<String, Object>> getWaitingList(
        ...
        try {
            ...
            Page<ConsultationListDTO> pageParam = new Page<>(page, pageSize);
            
            Page<ConsultationListDTO> result = consultationMapper.selectConsultationListByStatus(
                    pageParam, doctorId, "WAITING");
     ...
     * 获取进行中列表（医生端）
     */
    @Operation(summary = "获取进行中列表")
    @GetMapping("/api/consultations/in-progress")
    @RequireRole({"DOCTOR_PRIMARY", "DOCTOR_EXPERT"})
    public Result<Map<String, Object>> getInProgressList(
            @RequestParam(defaultValue = "1") Integer page,
            @RequestParam(defaultValue = "10") Integer pageSize) {
        try {
            Long doctorId = UserContext.getUserId();
            ...
            Page<ConsultationListDTO> result = consultationMapper.selectInProgressList(pageParam, doctorId);
    ...
    public Result<Map<String, Object>> getTodayStats() {
        try {
            ...
            waitingWrapper.eq(Consultation::getDoctorId, doctorId);
    ...
    public Result<Map<String, Object>> getConsultationDetail(@PathVariable Long consultationId) {
    ...
    public Result<Map<String, Object>> getConsultation(@PathVariable Long consultationId) {
        return Result.success(consultationService.getDetailById(consultationId));
    }
     ...
     */
    @Operation(summary = "结束问诊")
    @PutMapping("/api/consultations/{consultationId}/finish")
    @RequireRole({"DOCTOR_PRIMARY", "DOCTOR_EXPERT"})
    @Auditable(action = "FINISH_CONSULTATION", module = "CONSULTATION")
    public Result<Void> finishConsultationNew(@PathVariable Long consultationId) {
        consultationService.finishConsultation(consultationId);
        return Result.success();
    }
